...

{
    ...
    an_dir = fs->mkdir( outputFolderName_ );
...
SiStripSpyCMslope::beginJob()
{
    ...
    an_dir.make<TH1D>("CommonModeSlope", ";CM Slope;N APVs", slopeBins_, -1*slopeMax_, slopeMax_);
    an_dir.make<TH1D>("CommonModeAverage", ";Avg ADC counts / strip;N APVs", avgBins_, 0, avgMax_);
    an_dir.make<TH1D>("CommonModeStddev", ";ADC counts / strip std dev;N APVs", slopeBins_, 0, stddevMax_);
    an_dir.make<TH1D>("CommonModeSlopeUncert", ";CM Slope Uncert;N APVs", slopeBins_, 0, 0.06);
    an_dir.make<TH1D>("CommonModeSlopeRelUncert", "CM Slope Rel Uncert;N APVs", slopeBins_, 0, 10);

    // Making directories for each partition
    TFileDirectory TIB_dir = an_dir.mkdir("TIB");
    TFileDirectory TID_dir = an_dir.mkdir("TID");
    TFileDirectory TOB_dir = an_dir.mkdir("TOB");
    TFileDirectory TEC_dir = an_dir.mkdir("TEC");
    ...
    TFileDirectory id_dir = an_dir.mkdir("Det_IDs");

    // If you are checking IDs, make histograms for each ID
    std::stringstream id_str;
    if (checkIDs_.size())
    {
    if (DEBUG) std::cout << "Making det ID histograms" << std::endl;
    for (std::vector<uint32_t>::iterator Id=checkIDs_.begin(); Id!=checkIDs_.end(); Id++) {
        id_str.str(std::string());
        id_str << *Id;
        if (DEBUG) std::cout << "Making hist detId_" << id_str.str() << std::endl;
    ...
    if (allParts_)
    {
    if (DEBUG) std::cout << "Making histograms for all detector partitions" << std::endl;
    ...
    if (partitions_[ip] == "TIB") {
        ...
        if (DEBUG) std::cout << "making plot: " << partitions_[ip] << "/CommonModeSlope_" << hist_str.str() << std::endl;
    ...
    if (DEBUG) std::cout << "Finished begin job" << std::endl;
...
SiStripSpyCMslope::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    if (DEBUG) std::cout << "Getting tracker topology stuff\n";
    ...
    RUN_SLOPE_HIST = an_dir.getObject<TH1D>("CommonModeSlope","");
    RUN_AVG_HIST = an_dir.getObject<TH1D>("CommonModeAverage","");
    RUN_STDDEV_HIST = an_dir.getObject<TH1D>("CommonModeStddev","");
    RUN_UNCERT_HIST = an_dir.getObject<TH1D>("CommonModeSlopeUncert","");
    RUN_REL_UNCERT_HIST = an_dir.getObject<TH1D>("CommonModeSlopeRelUncert","");
    ...
    if (partitions_[ip] == "TIB") {
        hist_str.str(std::string());
        hist_str << partitions_[ip] << "_L" << il+1;
        if (DEBUG) std::cout << "making plot: " << partitions_[ip] << "/CommonModeSlope_Event_" << hist_str.str() << std::endl;
    ...
    //const std::string & name)
{
    ...
    TH1S * hist;
    if (type==POST_PEDESTAL)          hist = new TH1S("PostPedestal",        ";Strip number;ADC counts / strip",  768, 0, 768);
    else {
        hist = nullptr;
        return false;
    }
    ...
    {
        strip = static_cast<int>((*izs).strip());
        if (strip >= 768)
        {
            if (DEBUG) std::cout << "strip number " << strip << " is greater than 768. wtf??" << std::endl;
            break;
        }
        clusters[strip] = static_cast<int>((*izs).adc());
        if (DEBUG) std::cout << "FOUND CLUSTER: strip " << strip << ", ZS adc count is: " << (*izs).adc() << std::endl;
    }
	...
	if (DEBUG) std::cout << "subdetID is: " << subId << "\t layer is " << layer << "\tside is " << side<< endl;
    ...
    for (std::vector<uint32_t>::iterator Id=checkIDs_.begin(); Id!=checkIDs_.end(); Id++) {
        if (specifier == *Id) {
            is_match = true;
            if (DEBUG) std::cout << "Det Id Match!" << std::endl;
            break;
        }
    }
    ...
    */
    ...
    if (DEBUG) std::cout << "Number of hist entries is " << hist->GetEntries() << std::endl;
	double stddev = 0;
	double avg = 0;
    double slope = 0;
    double uncert = 0;
    int high_stddev = 0;
    double max_adc = 0;
    TF1 *f1 = new TF1("f1", "pol1", 0, 768);
	if (npairs != 0) {
	    for (uint32_t ii=0; ii < npairs; ii++) {
            if (DEBUG) std::cout << filled[ii] << std::endl;
            ...
            if ( filled[ii] <= minStrips_ ) {
                if (DEBUG) std::cout << "Something wrong, skipping APV" << std::endl;
                continue;
            }
            f1->Update();
            f1->SetRange(ii*nstrips+1, (ii+1)*nstrips);
		    //hist->Fit("f1", "Q", "R", ii*nstrips+1, (ii+1)*nstrips);
            //hist->Fit("pol1", "", "", ii*nstrips+1, (ii+1)*nstrips);
            // This seems to be the only consistent way to get the function to
            // fit only a range of strips
            hist->GetXaxis()->SetRangeUser(ii*nstrips+1, (ii+1)*nstrips);
		    ...
		    if (DEBUG) std::cout << "slope for strips " << nstrips*ii+1 << " - " << nstrips*(ii+1) << " is:  " << slope << " +/- " << uncert << std::endl;
            ...
            avg = hist->Integral(ii*nstrips+1,(ii+1)*nstrips)/filled[ii];
            if (DEBUG) cout << "Average ADC count is:\t" << avg << endl;
            AVG_HIST->Fill(avg);
            // stddev is calculated as:
            // sqrt( Sum( Avg**2 - Strip**2) ) / 128
		    for (uint32_t j=(ii*nstrips+1); j<=(ii+1)*nstrips;j++) {
    	    ...
    	    if (DEBUG) std::cout << "Standard dev is " << stddev << endl;
            ...
            if (stddev >= stddevLimit_) {
                if (DEBUG) std::cout << "Stddev is greater than limit, remembering detID" << std::endl;
                high_stddev++;
            }
            ...
            if (DEBUG) std::cout << "Filled APV" << std::endl;
